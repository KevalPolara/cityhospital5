export const INCREMENT_COUNTER='INCREMENT_COUNTER';
export const DECREMENT_COUNTER='DECREMENT_COUNTER';


export const GET_MEDICINE='GET_MEDICINE';
export const ADD_MEDICINE='ADD_MEDICINE';
export const UPDATE_MEDICINE='UPDATE_MEDICINE';
export const DELETE_MEDICINE='DELETE_MEDICINE';
export const LOADING_MEDICINE='LOADING_MEDICINE';
export const ERROR_MEDICINE='ERROR_MEDICINE';
export const ADD_TO_CART='ADD_TO_CART';
export const INDEX_CART='INDEX_CART';
export const INCREMENT_CART='INCREMENT_CART';
export const DECREMENT_CART='DECREMENT_CART';
export const DELETE_CART='DELETE_CART';
export const ADD_TO_WISHLIST='ADD_TO_WISHLIST';
export const DELETE_WISHLIST='DELETE_WISHLIST';
export const COLUR_WISHLIST='COLUR_WISHLIST';
export const REMOVE_WISHLIST = 'REMOVE_WISHLIST';


export const GET_DEPARTMENT='GET_DEPARTMENT';
export const ADD_DEPARTMENT='ADD_DEPARTMENT';
export const DELETE_DEPARTMENT='DELETE_DEPARTMENT';
export const UPDATE_DEPARTMENT='UPDATE_DEPARTMENT';


export const GET_DOCTOR='GET_DOCTOR';
export const ADD_DOCTOR='ADD_DOCTOR';
export const ERROR_DOCTOR='ERROR_DOCTOR';
export const DELETE_DOCTOR='DELETE_DOCTOR';
export const UPDATE_DOCTOR='UPDATE_DOCTOR';


export const SIGNUP_REQUEST = 'SIGNUP_REQUEST';
export const SIGNUP_RESPONSE = 'SIGNUP_RESPONSE';
export const AUTH_ERROR ='AUTH_ERROR';

export const LOGIN_REQUEST = 'LOGIN_REQUEST';
export const LOGIN_RESPONSE = 'LOGIN_RESPONSE';
export const LOGIN_ERROR = "LOGIN_ERROR";

export const FPASSWORDREQUEST = 'FPASSWORDREQUEST';
export const FPASSWORD_RESPONSE = 'FPASSWORD_RESPONSE';
export const FPASSWORD_ERROR = 'FPASSWORD_ERROR';

export const LOGOUT_REQUEST = 'LOGOUT_REQUEST';
export const LOGOUT_RESPONSE = 'LOGOUT_RESPONSE';
 



